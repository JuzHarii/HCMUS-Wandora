...
export function TripWorkspacePage() {
  ...
  const loadTrip = useCallback(async () => {
    ...
    try {
      ...
      setWorkspace({
        ...overview.workspace,
        current_user_role: overview.current_user_role,
      });
    } catch (e) {
      ...
      <div className="trip-workspace">
        <header className="workspace-view-header workspace-tabs-header">
          ...
          <nav className="workspace-tabs">
            <Link
              to="itinerary"
              className={`workspace-tab ${pathname.includes("/itinerary") ? "is-active" : ""}`}
            >
              <Map aria-hidden="true" /> Itinerary
            </Link>
            <Link
              to="packing"
            ...
            >
              <Briefcase aria-hidden="true" /> Packing
            </Link>
            <Link
              to="members"
            ...
            <Link
              to="reviews"
              className={`workspace-tab ${pathname.includes("/reviews") ? "is-active" : ""}`}
              style={isDraft ? { pointerEvents: "none", opacity: 0.5 } : {}}
            >
              <Star aria-hidden="true" /> Reviews
            </Link>
            <Link
              to="share"
              className={`workspace-tab ${pathname.includes("/share") ? "is-active" : ""}`}
              style={isDraft ? { pointerEvents: "none", opacity: 0.5 } : {}}
            >
              <Share2 aria-hidden="true" /> Share & Export
            </Link>
          </nav>
        </header>
