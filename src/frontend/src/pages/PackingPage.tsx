...
export function PackingPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const [overview, nextItems] = await Promise.all([
        getTripOverview(workspaceId),
        listPackingItems(workspaceId),
      ]);
    ...
    <WorkspaceShell>
      <div className="workspace-view packing-view">
        <header className="workspace-view-header">
          <div>
            <p className="dashboard-kicker">Trip preparation</p>
            <h1>Pack with the whole group.</h1>
            <p className="packing-subtitle">
              {workspace
                ? `${workspace.title} - ${workspace.destination}`
                : "Shared packing list"}
            </p>
          </div>
          {workspaceId && (
            <Link className="workspace-back-link" to={`/trips/${workspaceId}`}>
              <ArrowLeft aria-hidden="true" /> Itinerary
            </Link>
          )}
        </header>
        {isLoading && (
          ...
          <>
            <section className="packing-summary">
              ...
              <i aria-hidden="true">
                <b
                  style={{
                    width: `${items.length ? Math.round((completeCount / items.length) * 100) : 0}%`,
                  }}
                />
              </i>
              <button
                data-testid="add-custom-luggage-button"
            ...
            {items.length === 0 ? (
              <section className="packing-empty">
                ...
                <p>
                  Add the first item so the group can see what still needs
                  attention.
                </p>
                <button
                  className="dashboard-create-button"
                ...
                {items.map((item) => (
                  ...
                  >
                    <button
                      ...
                      type="button"
                      aria-label={`Mark ${item.name} ${item.is_completed ? "not ready" : "ready"}`}
                      onClick={() => void toggleItem(item)}
                    >
                      {item.is_completed && <Check aria-hidden="true" />}
                    </button>
                    <div>
                    ...
                    <span>
                      {item.assigned_to ? (
                        <>
                          <UsersRound aria-hidden="true" /> Assigned
                        </>
                      ) : (
                        "Unassigned"
                      )}
...
function PackingForm({
  ...
  workspaceId: string;
  onCancel: () => void;
  onCreated: (item: PackingItem) => void;
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      onCreated(
        await createPackingItem(workspaceId, {
          name: name.trim(),
  ...
  return (
    <form className="packing-form" onSubmit={submit}>
      <label>
        Item
        <input
