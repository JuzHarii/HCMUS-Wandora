    ...
    return (
      <li className="activity-row is-editing-row">
        <form className="manual-activity-form" onSubmit={submit} style={{ width: '100%' }}>
          ...
          <label>Notes<textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Optional tips, reminders..." rows={2} style={{ resize: 'vertical' }} /></label>
          <label>External Link<input type="url" value={externalUrl} onChange={(event) => setExternalUrl(event.target.value)} placeholder="https://..." /></label>
