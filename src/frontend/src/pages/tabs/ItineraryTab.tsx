...
export function ItineraryTab() {
  ...
  const loadTrip = useCallback(async () => {
    ...
    setError("");
    try {
      const [nextItinerary, nextVersions] = await Promise.all([
        workspacesApi.getItinerary(workspaceId),
        workspacesApi.listItineraryVersions(workspaceId),
      ]);
  ...
  async function regenerate() {
    if (
      ...
      !window.confirm(
        `Regenerate the AI suggestions? Your ${manualActivityCount} manual activit${manualActivityCount === 1 ? "y" : "ies"} will be kept.`,
      )
    )
      return;
    ...
    try {
      ...
      setError(
        generateError instanceof Error
          ...
          : "Could not regenerate the itinerary.",
      );
    } finally {
  ...
  async function initializeBlank() {
    if (
      !window.confirm(
        "Replace this itinerary with blank daily schedules? Existing activities will be removed.",
      )
    )
      return;
    ...
    try {
      ...
      setError(
        blankError instanceof Error
          ? blankError.message
          : "Could not initialize a blank itinerary.",
      );
    } finally {
  ...
  async function submitAdjustment(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      setItinerary(
        await workspacesApi.adjustItinerary(workspaceId, {
  ...
  async function restoreVersion(version: ItineraryVersion) {
    if (
      !window.confirm(
        "Restore this saved itinerary? The current itinerary will be replaced.",
      )
    )
      return;
    setIsRestoring(true);
    ...
    try {
      setItinerary(
        await workspacesApi.restoreItineraryVersion(workspaceId, version.id),
      );
  ...
  if (error && !itinerary)
    return (
      <section className="workspace-loading">
        ...
        <h1>We could not open this itinerary.</h1>
        <p>{error}</p>
        <button
  ...
  return (
    <div className="workspace-view workspace-itinerary-view">
      <section className="itinerary-hero">
        <div>
          <p className="eyebrow">
            <Compass aria-hidden="true" /> Itinerary draft
          </p>
          <p>
            {workspace.destination} ·{" "}
            {formatDateRange(workspace.start_date, workspace.end_date)} ·{" "}
            {workspace.group_size ?? 1} travelers
          </p>
        </div>
        <div className="itinerary-actions">
          <span
            className={`generation-source generation-source-${itinerary?.generation_source ?? "unknown"}`}
          >
            {formatGenerationSource(itinerary?.generation_source)}
          </span>
          <span className="status-pill">{workspace.status ?? "Draft"}</span>
          <div className="itinerary-action-buttons">
            <button
              className="button button-secondary itinerary-history-button"
              type="button"
            ...
            {canEdit && (
              ...
              >
                {isGenerating ? (
                  ...
                  <>
                    <Compass aria-hidden="true" /> Regenerate itinerary
                  </>
                )}
              </button>
            )}
      ...
      {error && (
        <div
          data-testid="adjustment-error-alert"
        ...
        >
          ...
          {canEdit && (
            <button
              ...
              type="button"
              onClick={() => void initializeBlank()}
              disabled={isInitializingBlank}
            >
              {isInitializingBlank
                ? "Preparing blank itinerary…"
                : "Start with a blank itinerary"}
            </button>
          )}
        </div>
      )}
      ...
      {canEdit && (
        ...
        >
          ...
          >
            <input
              data-testid="itinerary-adjustment-input"
              value={adjustPrompt}
            ...
            />
            <button
              data-testid="itinerary-adjustment-submit"
              className="button button-primary"
              type="submit"
            ...
            >
              {isAdjusting ? (
                ...
                <>
                  <Sparkles aria-hidden="true" /> Adjust with AI
                </>
              )}
            </button>
          </form>
      ...
      {isHistoryOpen && (
        <section
          className="itinerary-history"
          aria-label="Itinerary version history"
        >
          <div>
            <p className="dashboard-kicker">Version history</p>
            <h2>Restore a previous draft</h2>
            <p>Each AI regeneration saves the itinerary that came before it.</p>
          </div>
          {versions.length === 0 ? (
            <p className="history-empty">
              No previous versions yet. Regenerate once to create a restore
              point.
            </p>
          ) : (
            <ol>
              {versions.map((version) => (
                <li key={version.id}>
                  <div>
                    <strong>
                      {formatGenerationSource(version.generation_source)}
                    </strong>
...
function DayCard({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setError("");
    try {
      await itinerariesApi.addActivity({
        day_id: day.id,
        title: title.trim(),
  ...
  return (
    <article className="day-card">
      <header>
        ...
        <time>{formatDay(day.travel_date)}</time>
      </header>
          ...
          {day.activities.map((activity) => (
            ...
            >
              ...
              <div>
                ...
                {activity.notes && (
                  <p className="activity-note">{activity.notes}</p>
                )}
                {activity.external_url && (
                  <a
                    ...
                    rel="noreferrer"
                  >
                    Open map <ArrowUpRight aria-hidden="true" />
                  </a>
                )}
      ...
      {canEdit && (
        <div className={`blank-day ${isAddingActivity ? "is-editing" : ""}`}>
          {!isAddingActivity && (
            <button
          ...
          {isAddingActivity && (
            <form className="manual-activity-form" onSubmit={submit}>
              <label>
                Activity
                <input
              ...
              <label className="activity-location-field">
                Location
                <input
                  value={locationName}
              ...
              <label>
                Notes
                <textarea
                  data-testid="activity-notes-textarea"
                  value={notes}
                  onChange={(event) => setNotes(event.target.value)}
                  placeholder="Optional tips, reminders..."
                  rows={2}
                  style={{
                    width: "100%",
                    padding: "0.5rem",
              ...
              <label>
                External Link
                <input
                  data-testid="activity-url-input"
                  type="url"
                  value={externalUrl}
              ...
              {error && (
                <p
                  data-testid="activity-form-validation-alert"
