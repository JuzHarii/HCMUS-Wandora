...
def list_workspaces(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)) -> list[WorkspaceResponse]:
...
def get_workspace(workspace_id: Any, db: Session = Depends(get_db)) -> WorkspaceResponse:
    ws = workspace_service.get_workspace(db, workspace_id)
    return WorkspaceResponse.model_validate(ws)


@router.get("/{workspace_id}/overview", response_model=TripOverviewResponse)
def get_trip_overview(workspace_id: Any, db: Session = Depends(get_db)) -> TripOverviewResponse:
...
def initialize_blank_itinerary(workspace_id: Any, db: Session = Depends(get_db)) -> ItineraryResponse:
...
def list_itinerary_versions(workspace_id: Any, db: Session = Depends(get_db)) -> list[ItineraryVersionResponse]:
...
def restore_itinerary_version(workspace_id: Any, version_id: Any, db: Session = Depends(get_db)) -> ItineraryResponse:
...
async def adjust_itinerary(
    workspace_id: Any, payload: AdjustItineraryRequest, db: Session = Depends(get_db)
) -> ItineraryResponse:
